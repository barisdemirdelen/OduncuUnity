...
namespace Oduncu.ScaleUtil
{
    ...
    public class AnchorGameObject : MonoBehaviour
    {
        ...
        private IEnumerator m_UpdateAnchorRoutine; //Coroutine handle so we don't start it if it's already running

        // Use this for initialization
        private void Start()
        {
            m_UpdateAnchorRoutine = UpdateAnchorAsync();
            StartCoroutine(m_UpdateAnchorRoutine);
        }
        ...
        private IEnumerator UpdateAnchorAsync()
        {
            ...
            {
                print($"CameraAnchor found CameraFit instance after waiting {cameraWaitCycles} frame(s). " +
                      "You might want to check that CameraFit has an earlie execution order.");
            }

            UpdateAnchor();
            m_UpdateAnchorRoutine = null;
        }
        ...
        private void Update()
        {
            if (m_UpdateAnchorRoutine != null || !executeInUpdate) return;
            m_UpdateAnchorRoutine = UpdateAnchorAsync();
            StartCoroutine(m_UpdateAnchorRoutine);
        }
#endif
